class Solution:
    def PredictTheWinner(self, nums: 'List[int]') -> bool:
        ...
        def method_one():
            ...
            def recursion_method(start_i, end_i):
                ...
                left_score = nums[start_i] - recursion_method(start_i + 1,
                                                     end_i)  # player 1 choose start_i and - player's max score in range(start_i + 1, end_i)
                right_score = nums[end_i] - recursion_method(start_i, end_i - 1)
                memo[start_i][end_i] = max(left_score, right_score)

                return memo[start_i][end_i]

            return recursion_method(0,len(nums) - 1) >= 0
